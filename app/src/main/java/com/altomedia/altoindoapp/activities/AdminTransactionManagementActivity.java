...
public class AdminTransactionManagementActivity extends AppCompatActivity {
    ...
    private void loadWithdrawals() {
        db.collection("withdrawals").whereEqualTo("status", "pending").get().addOnSuccessListener(querySnapshot -> {
    ...
    private void updateWithdrawStatus(String docId, String status) {
        db.collection("withdrawals").document(docId).update("status", status).addOnSuccessListener(aVoid -> {
